...
do
local Arian = 304331223 --put your id here(BOT OWNER ID)
local Sosha = 267081450
--local Sosha2 = 304331223
   ...
   Username = '----'
    ...
    if not value then
	 if result.peer_id == tonumber(Arian) then
	   ...
	   text = text..'Rank :مدیر ارشد ربات⭐️⭐️⭐️⭐️⭐️ (Full Access Admin) \n\n'
	  ...
	  elseif is_admin2(result.peer_id) then
	   text = text..'مقام : ادمین⭐️⭐️ \n\n'
	  elseif is_owner2(result.peer_id, extra.chat2) then
	   text = text..'مقام : مدیر گروه⭐️⭐️⭐️ \n\n'
	  elseif is_momod2(result.peer_id, extra.chat2) then
	    text = text..'مقام : مدیر⭐️⭐️⭐️⭐️ \n\n'
      else
	    text = text..'مقام : کاربر⭐️ \n\n'
	 end
   else
 ...
 if result.username then
  ...
  if not value then
	 if result.peer_id == tonumber(Arian) then
	   text = text..'مقام : BOT Creator \n\n'
	   elseif result.peer_id == tonumber(Sosha) then
	   text = text..'مقام : مدیر ارشد ربات⭐️⭐️⭐️⭐️⭐️ (Full Access Admin) \n\n'
	   elseif result.peer_id == tonumber(Sosha2) then
	   text = text..'مقام : مدیر ارشد ربات⭐️⭐️⭐️⭐️⭐️ (Full Access Admin) \n\n'
	  elseif is_admin2(result.peer_id) then
	   text = text..'مقام : ادمین⭐️⭐ \n\n'
	  elseif is_owner2(result.peer_id, extra.chat2) then
	   text = text..'مقام : مدیر گروه⭐️⭐️⭐️ \n\n'
	  elseif is_momod2(result.peer_id, extra.chat2) then
	   text = text..'مقام : مدیر⭐️⭐️⭐️⭐️ \n\n'
		 ...
		 if not value then
		    if result.from.peer_id == tonumber(Arian) then
		     ...
		     elseif is_owner2(result.from.peer_id, result.to.id) then
		       text = text..'مقام : لیدر گروه⭐️⭐️⭐️ \n\n'
