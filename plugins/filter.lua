local function filter(msg, name)
    local hash = 'chat:'..msg.to.id..':filters'
...
local function run(msg, matches)
  ...
  local text = filter(msg, name)
  return text
  end
  if matches[2] == 'filters' then
  return list_variablesbad(msg)
  elseif matches[2] == 'cf' then
if not is_momod(msg) then return '👍' end
  local asd = '1'
    return clear_commandbad(msg, asd)
  elseif matches[2] == 'remfilter' or matches[2] == 'rw' then
   if not is_momod(msg) then return '👍' end
