...
if matches[1] == "dl" and matches[2] and matches[3] then
  ...
  -------------------------
   ...
   file:flush()
   file:close()
   return "پلاگین با موفقیت ذخیره شد. "
end
------------------------
   --------------------------
...
if matches[1] == 'voice' and is_sudo(msg) then
 local text = matches[2]
  ...
  local receiver = get_receiver(msg)
  local file = download_to_file(url,'─═ঈঊ(➊)ঊঈ═─.mp3')
...
------------------------
 local receiver = get_receiver(msg)
    ...
    send_large_msg(receiver, '─═ঈঊ(➊)ঊঈ═─', ok_cb, false)
